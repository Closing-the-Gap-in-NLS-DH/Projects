!(function () {
  ...
  (Timesheet.prototype.insertData = function () {
    for (
      var html = [],
        widthMonth = this.container.querySelector(".scale section").offsetWidth,
        n = 0,
        m = this.data.length;
      m > n;
      n++
      ...
      var cur = this.data[n],
        ...
        line = [
          '<span style="margin-left: ' +
            ...
            '" data-duration="' +
            (cur.end
              ? Math.round((cur.end - cur.start) / 1e3 / 60 / 60 / 24 / 39)
              : "") +
            '"></span>',
    ...
    (Timesheet.prototype.drawSections = function () {
      ...
      (this.container.className = "timesheet color-scheme-default"),
        (this.container.innerHTML =
          '<div class="scale">' + html.join("") + "</div>");
    }),
    (Timesheet.prototype.parseDate = function (date) {
      return (
        -1 === date.indexOf("/")
          ? ((date = new Date(parseInt(date, 10), 0, 1)), (date.hasMonth = !1))
          : ((date = date.split("/")),
            (date = new Date(
              parseInt(date[1], 10),
        ...
        var beg = this.parseDate(data[n][0]),
          ...
          cat =
            4 === data[n].length
              ...
              ? data[n][2]
              : "default";
        beg.getFullYear() < this.year.min &&
          (this.year.min = beg.getFullYear()),
          end && end.getFullYear() > this.year.max
            ? (this.year.max = end.getFullYear())
            : beg.getFullYear() > this.year.max &&
  ...
  (Bubble.prototype.formatMonth = function (num) {
    ...
    (Bubble.prototype.getStartOffset = function () {
      return (
        (this.widthMonth / 12) *
        (12 * (this.start.getFullYear() - this.min) + this.start.getMonth())
      );
    }),
    (Bubble.prototype.getFullYears = function () {
      return (
        ((this.end && this.end.getFullYear()) || this.start.getFullYear()) -
        ...
        this.end
          ? this.end.hasMonth
            ? ((months += this.end.getMonth() + 1),
              (months +=
                12 - (this.start.hasMonth ? this.start.getMonth() : 0)),
              (months += 12 * (fullYears - 1)))
            : ((months +=
                12 - (this.start.hasMonth ? this.start.getMonth() : 0)),
              (months += 12 * (fullYears - 1 > 0 ? fullYears - 1 : 0)))
          : (months += this.start.hasMonth ? 1 : 12),
        months
      );
    }),
    ...
    (Bubble.prototype.getDateLabel = function () {
      return [
        (this.start.hasMonth
          ? this.formatMonth(this.start.getMonth() + 1) + "/"
          : "") + this.start.getFullYear(),
        this.end
          ? "-" +
            ((this.end.hasMonth
              ? this.formatMonth(this.end.getMonth() + 1) + "/"
              : "") +
